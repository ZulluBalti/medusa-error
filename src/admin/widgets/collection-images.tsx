...
const CollectionImagesWidget = ({
  data,
}: DetailWidgetProps<HttpTypes.AdminCollection>) => {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const queryClient = useQueryClient()
  const queryKey = ["collection-images", data.id]

  const { data: imagesData, isLoading } = useQuery({
    queryKey,
    queryFn: () =>
      sdk.client.fetch<{ images: CollectionImage[] }>(
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      return sdk.client.fetch(`/admin/collections/${data.id}/images`, {
        method: "POST",
  ...
  const deleteMutation = useMutation({
    mutationFn: (imageId: string) =>
      sdk.client.fetch(`/admin/collections/${data.id}/images/${imageId}`, {
        method: "DELETE",
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey })
